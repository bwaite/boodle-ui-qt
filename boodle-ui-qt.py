...
def qtmain():
    ...
    class Window(QtGui.QWidget):
        def __init__(self, app):
            ...
            self.stopTime = QtGui.QDateTimeEdit(QtCore.QDateTime.currentDateTime().addSecs(3600 * 8))
            self.stopTime.setDisplayFormat("ddd MMM d, h:mm AP")

            # filter pattern signals
            self.connect(self.stopTime,
                    QtCore.SIGNAL('timeChanged(const QTime &)'),
                    self.checkStopTime)
            
            self.timer = QtCore.QTimer(self)
            self.connect(self.timer, QtCore.SIGNAL("timeout()"), self.checkStopTime)
            ...
            selectedLayoutH.addWidget(self.nowPlayingButton)
            selectedLayoutH.addWidget(self.selectedButton)
            selectedLayoutH.insertStretch(2)
            ...
            proxyLayout.addWidget(QtGui.QLabel("Stop Time:"))
            proxyLayout.addWidget(self.stopTime)
        ...
        def stopPlay(self):
            ...
            self.nowPlayingButton.setEnabled(False)
            self.nowPlayingLabel.setText("Double-click a soundscape to play\n")
        ...
        def checkStopTime(self):
            if QtCore.QDateTime.currentDateTime() >= self.stopTime.dateTime():
                self.stopPlay()
